...
class DeviceScenario {
  ...
  final Key key;
  final AfterPump afterPump;
  final _DeviceScenarioWidget widget;
}

/// DeviceBuilder builds [Device] size driven layout for its children
class DeviceBuilder {
  /// Factory to create scenarios rendered on multiple device sizes in a widget
  ...
  factory DeviceBuilder({
    WidgetWrapper wrap,
    Color bgColor,
  }) {
    return DeviceBuilder._(
      wrap: wrap,
      bgColor: bgColor,
    );
  }
  ...
  Size get requiredWidgetSize {
    ...
    if (scenarios.length > _numberOfDevicesPerScenario) {
      final scenariosPerDevice =
          (scenarios.length / _numberOfDevicesPerScenario);
